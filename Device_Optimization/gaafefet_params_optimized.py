...
# =============================================================================
DEVICE = {
    "Vth_virgin": 0.018,          # V  erased-state Vth (fine I-V, mwfine node)
    "Vth_fire": -0.318,           # V  programmed-state Vth (fine I-V)
    "SS": 62.3e-3,                # V/dec  subthreshold slope (near-ideal; 5 nm body)
    "ID_baseline": 4.43e-10,      # A  erased read current @ VGS_read=0 V (fine I-V)
    "ID_fire": 1.359e-8,          # A  read current after 9 pulses (LTP)
    "MW": 0.336,                  # V  memory window = Vth_virgin - Vth_fire
    # V_t / SS / MW are voltages and are unchanged by the width fix.  The V_t
    # criterion is I_cc = 6.34e-3 uA/um in the corrected convention, which is the
    # same PHYSICAL current as the 1e-2 uA/um used before it.  Quoting a round
    # 1e-2 uA/um on the corrected axis instead would give V_t = +0.049 / -0.306 V
    # and MW = 0.355 V: a criterion choice, not a physics change.  The erased
    # branch crosses inside its ambipolar recovery (local slope ~159 mV/dec), so
    # its V_t is the criterion-sensitive one.
}
...
LIF = {
    ...
    "R_on_estimate": 3.68e6,      # Ω  VDS/ID_fire = 0.05V / 1.359e-8 A
    "R_off_estimate": 1.129e8,    # Ω  VDS/ID_baseline = 0.05V / 4.43e-10 A
    "g_min": 8.86e-9,             # S  1/R_off -- the SNN synapse's low weight
    "g_max": 2.72e-7,             # S  1/R_on at pulse 9 (the fire criterion)
    "g_max_p15": 1.50e-5,         # S  1/R at pulse 15, the top analog level
...
def print_parameters():
    ...
    print(f"Memory window {DEVICE['MW']:.3f} V | SS {DEVICE['SS']*1e3:.1f} mV/dec | "
          ...
          f"retained ON/OFF 3137x | E ~{PENDING['E_spike']*1e15:.4f} fJ/pulse")
