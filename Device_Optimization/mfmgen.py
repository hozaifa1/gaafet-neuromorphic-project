...
* Quasi-static in the physical sense instead: a quarter period of 25 us = 25
  tau_E, so the loop is rate-independent (tau_E = 1e-6 s in app.par).
* 3 wake-up cycles before the measured one.  A fresh Preisach model starts from
  a depolarized state and the first loop is not representative -- this is the
  numerical analogue of the wake-up cycling done on real HZO capacitors.
...
Math {{ Extrapolate RelErrControl Digits=6 Notdamped=50 Iterations=50 Transient=BE
...
LEG = """  NewCurrentPrefix="{prefix}"
  Transient ( InitialTime={t0:.6e} FinalTime={t1:.6e}
    InitialStep={dt:.6e} MaxStep={dt:.6e} MinStep={dtmin:.6e} Increment=1.2
    Goal{{ Name="top_metal" Voltage={v} }} ) {{ Coupled(Iterations=100) {{ Poisson }}{plot} }}
"""


def gen(vmax=1.96, node="mfm_pe196", par="app_iv_fe07b.par", t_quarter=25e-6,
        n_wakeup=3, npts=200, t_fe_um=0.007, l_cap_um=0.100):
    """Triangular P-E loop: 0 -> +V, n_wakeup full cycles, then one measured cycle."""
    dt = t_quarter / 40.0            # 40 solver steps per quarter on the wake-up legs
    ...
    for prefix, v in (("down_", -vmax), ("up_", vmax)):
        ...
        s += LEG.format(prefix=prefix, t0=t, t1=t1, dt=dt_meas, dtmin=dt_meas / 1e4,
                        v=v, plot=plot)
        t = t1
...
if __name__ == "__main__":
    for vm, node in ((1.96, "mfm_pe196"), (3.0, "mfm_pe300")):
        ...
        assert c.count("NewCurrentPrefix") == 1 + 2 * 3 + 2
