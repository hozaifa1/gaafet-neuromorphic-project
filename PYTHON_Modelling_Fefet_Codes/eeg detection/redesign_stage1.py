...
@torch.no_grad()
def evaluate(model, test_loader, device, quant, rmode="last", k=116, classes=("N", "F", "SVEB", "VEB")):
    DEV.quant = quant
    model.eval()
    y_true, y_pred = [], []
    ...
    rep = ecg_report(y_true, y_pred, classes=list(classes))
    if len(classes) == 2:   # binary (EEG epilepsy): imbalanced test -> report Se/Sp/G-mean
        sens = rep["per_class"][classes[1]]["Se"]    # recall of positive class (epileptic)
        spec = rep["per_class"][classes[0]]["Se"]    # recall of negative class (normal)
        rep["sensitivity"] = sens
        rep["specificity"] = spec
        rep["g_mean"] = float((sens * spec) ** 0.5)
    return rep
...
def main():
    ...
    ap.add_argument("--input_scaling", type=float, default=9900e-6,
                    help="neuron input transconductance (drive). ECG(3ch)=9900e-6; EEG(37ch dense) needs lower")
    ...
    ap.add_argument("--data", choices=["intra", "interpatient", "intra2lead", "inter2lead", "intra1lead", "eeg"],
                    default="intra",
                    help="npz datasets: interpatient (1-lead VO2), intra2lead/inter2lead/intra1lead (delta, ALLDEVICE), "
                         "eeg (CHB-MIT epilepsy, 37-ch binary, eeg.npz)")
    ...
    NPZ = {"interpatient": os.path.join(_HERE, "interpatient_data.npz"),
           ...
           "eeg": os.path.join(_HERE, "eeg.npz")}
    if args.data in NPZ:
        npz = np.load(NPZ[args.data])
        F_["num_in"] = int(npz["X_train"].shape[-1])          # auto: 3 (1-lead) / 5 (2-lead) / 37 (eeg)
        F_["num_out"] = int(npz["y_train"].max()) + 1         # auto: 4 (ECG AAMI) / 2 (EEG binary)
        print(f"[data] {args.data} npz | {F_['num_in']} input channels | {F_['num_out']} classes")
        ...
        cw = torch.Tensor(compute_class_weight('balanced', classes=np.arange(F_["num_out"]),
    ...
    if args.model == "orig":
        ...
        model = VO2LSNN(
            ...
            input_scaling=args.input_scaling, dt=F_["dt"], max_delay=F_["max_delay"], refractory=5,
    ...
    if args.probe:
        ...
        print("[probe] init firing rate vs input_scaling (drive); target ~10-50 Hz:")
        base = args.input_scaling
        for isc in [base, base/3, base/10, base/30, base/100, base/300]:
            model.hidden.sub_module_1.input_scaling = isc
            model.hidden.sub_module_2.input_scaling = isc
            with torch.no_grad():
                model(ecg.float()); fr = model.spike_for_reg.mean().item() / F_["dt"]
                functional.reset_net(model)
            print(f"  input_scaling={isc:10.2e} -> init FR = {fr:8.1f} Hz")
        return
    ...
    history = []
    best_macro_f1 = -1.0
    # metric names / selection: EEG (binary) is imbalanced -> select best checkpoint by G-mean
    CLASSES = ("N", "E") if F_["num_out"] == 2 else ("N", "F", "SVEB", "VEB")
    IS_EEG = (F_["num_out"] == 2)
    t0 = time.time()
    for epoch in range(epochs):
        ...
        rep_dev = evaluate(model, test_loader, device, quant=True, rmode=args.readout, k=F_["out_cue"], classes=CLASSES)
        last = (epoch == epochs - 1)
        rep_fp = evaluate(model, test_loader, device, quant=False, rmode=args.readout, k=F_["out_cue"], classes=CLASSES) if last else None
        ...
        if IS_EEG:
            print("  [EEG]  G-mean=%.4f  sensitivity=%.4f  specificity=%.4f"
                  % (rep_dev["g_mean"], rep_dev["sensitivity"], rep_dev["specificity"]))
        ...
        sel_metric = rep_dev["g_mean"] if IS_EEG else rep_dev["macro_f1"]   # EEG: pick best by G-mean
        if sel_metric >= best_macro_f1:
    ...
    print(f"  best device {'G-mean ' if IS_EEG else 'macro-F1'} : {best_macro_f1:.4f}")
    if IS_EEG:
        best_gm = max((h["device"].get("g_mean", 0.0) for h in history), default=0.0)
        bh = max(history, key=lambda h: h["device"].get("g_mean", 0.0))
        print(f"  best G-mean epoch    : {bh['epoch']} (sens={bh['device']['sensitivity']:.4f}, "
              f"spec={bh['device']['specificity']:.4f})")
