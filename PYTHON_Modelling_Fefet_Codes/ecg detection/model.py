...
class eLIFVO2(neuron.BaseNode):
    """
    LIF neuron with GAA FeFET-equivalent RC membrane dynamics.

    The original VO2 resistive element is replaced by the GAA FeFET channel:
      Rh  → R_off = 5250 Ω  (off-state / baseline channel resistance, SimC v6 6V node)
      Rs  → R_on  = 2580 Ω  (on-state / fired channel resistance, SimC v6 6V node)
      Cmem back-calculated so tau = Cmem*(Rh+Rs) = 11.11 ms (identical to VO2 value)
      input_scaling rescaled so gain = (Rh+Rs)*input_scaling ≈ 77.5 (identical to VO2 value)
    The RC charge equation, fire/reset logic, and surrogate gradient are unmodified.
    """
    def __init__(self,
                 Rh: float = 5250., Rs: float = 2580.,   # FeFET R_off and R_on (Ω), SimC v6 6V node
                 v_threshold: float = 3.6, v_reset: float = 1.5,
                 Cmem: float = 1.419e-6,                  # back-calc: tau/(Rh+Rs) = 11.11e-3/7830
                 Vdd: float = 5.,
                 input_scaling: float = 9900e-6,           # rescaled: preserves gain=(Rh+Rs)*input_scaling≈77.5
    ...
    def neuronal_charge(self, x: torch.Tensor):
        # FeFET input path is unipolar; the MOSFET on the input node blocks negative drive
        x = torch.relu(x)
        # RC membrane update: v = factor*v + (1-factor)*(Rh+Rs)*I_in
        # Rh+Rs = R_off+R_on = 7830 Ω; factor = exp(-dt/tau) = 0.9512 (tau=11.11ms, unchanged)
...
class ALIFVO2(neuron.BaseNode):
    """
    Adaptive LIF neuron with GAA FeFET-equivalent RC membrane dynamics.

    Same FeFET substitution as eLIFVO2 (Rh→R_off, Rs→R_on, recalibrated Cmem and
    input_scaling). The frequency-adaptation circuit — a MOSFET-based spike-feedback
    path that modulates a membrane leak current (Il) — is unchanged, because it is a
    peripheral CMOS circuit independent of the FeFET device itself.

    Adaptation mechanism:
      - Each spike charges va via pMOS Ia (neuronal_adapt)
      - va drives nMOS leak Il subtracted from the input drive (neuronal_charge)
      - High sustained firing → large va → large Il → membrane suppressed (adaptation)
    """
    def __init__(self,
                 Rh: float = 5250., Rs: float = 2580., Ra: float = 100e3,  # FeFET R_off, R_on; MOSFET Ra unchanged
                 v_threshold: float = 3.6, v_reset: float = 1.5,
                 Cmem: float = 1.419e-6, Ca: float = 7e-6,                  # Cmem back-calc: tau/(Rh+Rs)=11.11ms
                 vtn: float = 0.745, vtp: float = 0.973, kappa_n: float = 29e-6, kappa_p: float = 18e-6,
                 wl_ratio_n: float = 6., wl_ratio_p: float = 4.,
                 Vdd: float = 5.,
                 input_scaling: float = 9900e-6,                            # rescaled: preserves gain≈77.5
                 dt: float = 1e-3, refractory: int = 5,
                 surrogate_function: Callable = neuron.surrogate.Sigmoid(),
                 detach_reset: bool = False):
        super().__init__(v_threshold, v_reset, surrogate_function, detach_reset)
    ...
    def neuronal_charge(self, x: torch.Tensor):
        # FeFET input path is unipolar; the MOSFET on the input node blocks negative drive
        x = torch.relu(x)
        # nMOS leak current controlled by adaptation voltage va; subtracts from input drive
        # Il grows when va > vtn, which happens after sustained high-frequency spiking
        Il = absId(kappa=self.kappa_n, w_over_l=self.wl_ratio_n, vgs=self.va,
                   vth=self.vtn, vds=self.v)
        # RC membrane update with adaptation: v = factor*v + (1-factor)*(Rh+Rs)*(I_in - Il)
...
class VO2LSNN(nn.Module):
    """
    LSNN with GAA FeFET membrane dynamics (LIF and ALIF neurons) and MOSFET adaptation.
    Architecture: Input -> Delayed FC -> Recurrent [FeFET-LIF | FeFET-ALIF] -> LP filter -> Output

    The spiking neurons use FeFET-derived RC parameters (Rh=R_off, Rs=R_on, recalibrated Cmem
    and input_scaling) while preserving tau=11.11ms and gain≈77.5 from the original VO2 model.
    All MOSFET adaptation, training, and evaluation logic are unchanged.
