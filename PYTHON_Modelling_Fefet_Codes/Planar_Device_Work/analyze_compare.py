...
def main():
    ...
    # paper-quality figures live in plot_planar.py (fig_*.png); run that for the plots.
...
- `plots/fig_spike_train_gaa.png` / `_planar.png` / `_compare.png` — the temporal waveform:
  input spike train + non-volatile synaptic-weight potentiation (device = analog synapse,
  NOT a self-resetting neuron; holds between spikes, reset = explicit erase pulse)
