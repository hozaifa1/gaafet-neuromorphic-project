...
predicted outcome. It is the check that the area-factor correction of
Section~\ref{sec:norm} was a change of normalization, not of physics.
...
Sweeping the number of conductance levels from two to fifteen gives a curve
(Fig.~\ref{fig:k2}) that is
...
over the same range. They agree, so the non-monotonicity is not an artefact of how
...
shape error (Fig.~\ref{fig:k3c}) gives median shifts of \num{0.51} decades and a median residual scatter of
...
Injecting the measured cycle-to-cycle noise per conductance branch (Fig.~\ref{fig:k3b}), the fifteen-level
...
\num{0.036} accuracy. That is the wrong trade, and the reason matters: it
is a design-methodology point, not a device one. The separability criterion returns
...
per-device gain trim, and the three-sigma separability of the cycle-to-cycle run.
...
bit-identical, so a handful of weights land on the other side of a bucket edge. The
deployment is boundary-sensitive at the one-beat level, which is worth knowing but not
worth reading into.
