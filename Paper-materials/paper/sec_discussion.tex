...
It is worth being explicit that this ordering was not expected. Ferroelectric thickness
sets the coercive voltage, so thickness control looks like the obvious lever, and a study
that measured only thickness sensitivity would have concluded that the device was
well-behaved while leaving the dominant term unmeasured.
...
has little system-level value until the conversion overhead is addressed, and that is a
more useful thing for a reader to know than a smaller number would have been.
...
degradation that was entirely the post-write depolarization transient being read as data
...
are worth repeating: form a ratio only from measurements taken in the same run, and treat a
