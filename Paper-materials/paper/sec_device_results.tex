...
set where the device can be read, and Fig.~\ref{fig:panel8}(d) turns them into a design constraint.
...
line to document the disagreement. Computed values: \fignote{I1}; \fignote{I2};
\fignote{I3}.}
\label{fig:panel8}
\end{figure*}
...
Figure~\ref{fig:panel8}(c) is the ferroelectric thickness, and it is a Pareto choice rather than an
...
Figure~\ref{fig:panel8}(d) is the read bias. The retained window is largest near $V_G=0$ while the rest
...
source number could not be confirmed in the paper text. Computed values: \fignote{J1};
\fignote{J5}.}
\label{fig:panel10}
\end{figure*}
...
Figure~\ref{fig:panel10}(b) places this device on a survey of fifteen recent ferroelectric analog-memory
