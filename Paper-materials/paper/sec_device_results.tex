...
marked at both its open-loop and its write-verify count. Open markers are counts that
could not be confirmed in the paper text. Computed values: \fignote{J1}; \fignote{J5}.}
\label{fig:panel10}
\end{figure*}
...
papers state a count, and those run from sixteen to a hundred and twenty-eight against this
...
placement does. A device reporting a hundred and twenty-eight levels has not thereby shown
...
digital object identifier and the sentence each number was read from. Counts that could not
be confirmed in the paper text are drawn as open markers; six of the fifteen rows carry
that caveat and are listed in \texttt{literature\_benchmark\_notes.md}.
