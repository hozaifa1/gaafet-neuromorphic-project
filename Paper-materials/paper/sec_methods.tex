...
$N_\mathrm{sd}=\SI{5e19}{\per\cubic\centi\metre}$. Panel~1(a) shows the stack to scale,
...
The trajectory is in panel~8(a) and the individual sweeps in panels~8(b)--(c).
...
Panel~2 shows the result three ways: the overlay itself, the same curves with the
...
(\ref{eq:tesw}). Panel~1(b) draws the mapping.
...
gate-all-around nanosheet has corners, and the electrostatics at those corners --- and
...
\item \textbf{A retained value must be read after the state has settled, and the delay
  ...
  retention collapse, a read-disturb collapse of \SI{-84.7}{\percent}, and an endurance
