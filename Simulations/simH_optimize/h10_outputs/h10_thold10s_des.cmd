*===================================================================
*== PHASE 1D - H10 RETENTION SWEEP (10 at V_GS = 0 V)
*==
*== Single 9-pulse fire burst at V_pgm = 2.0 V, then hold at V_GS = 0 V
*== for 10 seconds (Quasistationary -> Sentaurus pseudo-time).
*== Final 100 ns transient read at V_GS = -0.5 V to capture retained ID.
*==
*== SWB sweep: 10 in {10, 100, 1000} s (3 nodes).
*==
...
File {
    ...
    Plot       = "h10_outputs/h10_thold10s_des.tdr"
    Current    = "h10_outputs/h10_thold10s_des.plt"
    Output     = "h10_outputs/h10_thold10s_des.log"
}
...
Solve {
  ...
  *=== HOLD at V_GS = 0 V for 10 s (Transient, time-driven) ===
  NewCurrentPrefix="retention_hold_"
  Transient (
    InitialTime=1.8280000000e-06 FinalTime=1.000000182800000e+01
    InitialStep=1e-9 MaxStep=5.000000000000000e-01 MinStep=1e-12
    Increment=1.4
  ) { Coupled (Iterations = 100) {Poisson Electron Hole} }
  NewCurrentPrefix="post_hold_ramp_"
  Transient (
    InitialTime=1.000000182800000e+01 FinalTime=1.000000183800000e+01
    InitialStep=1e-3 MaxStep=5e-2 MinStep=1e-7
    Increment=1.4
    Goal { Name="gate_contact" Voltage= -0.5 }
  ) { Coupled (Iterations = 100) {Poisson Electron Hole} }
  ...
  Transient (
    InitialTime=1.000000183800000e+01 FinalTime=1.000000193800000e+01
    InitialStep=1e-11 MaxStep=5e-9 MinStep=1e-15
    Increment=1.4
  ) { Coupled (Iterations = 100) {Poisson Electron Hole}
      CurrentPlot( Time = (Range=(1.000000183800000e+01 1.000000193800000e+01) Intervals=20) ) }
