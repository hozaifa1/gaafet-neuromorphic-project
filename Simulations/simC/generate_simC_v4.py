"""Generate sdevice_simC_v4.cmd — fine Vpulse sweep in sub-fire regime.

v3 Results:
  Vpulse=3V, pw=1us → 1.80x on P1 (near-fire on first pulse)
  Vpulse=4V, pw=1us → 2.04x on P1 (FIRE on first pulse)
  Vpulse=5V, pw=1us → 2.24x on P1 (FIRE on first pulse)

v4 Goal: Find the Vpulse range where integration is *gradual* over
multiple pulses (5-10 pulses to reach 2x fire threshold).
  → Sweep Vpulse = 1.5, 2.0, 2.5 V with pw=1us
  → Keep tau_E=1us (par file), Vreset=-6V
"""
import os

# ── Parameters ──
VPULSE_VAR = "@Vpulse@"   # SWB sweep: 1.5, 2.0, 2.5 V
VRESET = -6.0              # Fixed strong reset
VGS_READ = 0.20
VDS = 0.05
PW = 1.0e-6               # 1us write pulse (same as v3)
TR = 1.0e-9               # 1ns rise
TF = 1.0e-9               # 1ns fall
T_READ = 1.0e-7           # 100ns read hold
N_PULSES = 10
LEAK_AFTER = 5             # Insert leak gap after this pulse
LEAK_DUR = 5.0e-6          # 5us leak gap
RESET_HOLD = 1.0e-6        # 1us reset hold

CYCLE = TR + PW + TF + T_READ  # 1.102us per pulse
...
# ── Header ──
w("*===================================================================")
w("*== PHASE 1B — SIM C v4: FINE VPULSE SWEEP FOR GRADUAL INTEGRATION")
w("*== v3 showed 3V fires near P1, 4-5V fires ON P1.")
w("*== v4 sweeps lower Vpulse (1.5, 2.0, 2.5V) to find multi-pulse fire.")
w(f"*== Sweep: {VPULSE_VAR} = 1.5, 2.0, 2.5 V  (SWB nodes)")
w(f"*== Fixed: VGS_read={VGS_READ}V, VDS={VDS}V, Vreset={VRESET}V, pw={PW*1e6:.0f}us")
w(f"*== Protocol: {N_PULSES} write-then-read cycles, {LEAK_DUR*1e6:.0f}us leak gap after P{LEAK_AFTER}")
w(f"*== Each cycle: {TR*1e9:.0f}ns rise + {PW*1e6:.0f}us write + {TF*1e9:.0f}ns fall + {T_READ*1e9:.0f}ns read")
w("*===================================================================")
w()
...
w("  Quasistationary (")
w(f"    InitialStep=1e-2 MaxStep=0.1 MinStep=1e-6")
...
w(f"  * pw={PW*1e6:.0f}us, Vpulse={VPULSE_VAR}, VGS_read={VGS_READ}V")
...
# Write output
outpath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sdevice_simC_v4.cmd")
