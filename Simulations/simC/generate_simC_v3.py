"""Generate sdevice_simC_v3.cmd with correct timing.
Run once to produce the cmd file, then delete this script."""
import os
...
VGS_READ = 0.20
VDS = 0.05
PW = 1.0e-6               # 1us write pulse (10x longer than v2)
TR = 1.0e-9               # 1ns rise
TF = 1.0e-9               # 1ns fall
...
# ── Header ──
w("*===================================================================")
w("*== PHASE 1B — SIM C v3: STRONGER WRITE-THEN-READ LIF FIRE")
w("*== Fix: Vpulse swept higher (3-5V) + pw=1us (10x v2)")
w(f"*== Sweep: {VPULSE_VAR} = 3.0, 4.0, 5.0 V  (SWB nodes)")
w(f"*== Fixed: VGS_read={VGS_READ}V, VDS={VDS}V, Vreset={VRESET}V")
...
# Write output
outpath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sdevice_simC_v3.cmd")
