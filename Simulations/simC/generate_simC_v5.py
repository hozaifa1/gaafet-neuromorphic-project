"""Generate sdevice_simC_v5.cmd — short pulses + high Vpulse for gradual integration.

v3 Results (pw=1us):
  Vpulse=3V → P1=1.80x, P10=1.93x (near fire, saturates by P3)
  Vpulse=4V → P1=2.04x (FIRE on P1)
  Vpulse=5V → P1=2.24x (FIRE on P1)

v4 Results (pw=1us):
  Vpulse=1.5V → P1=1.35x, P10=1.43x (no fire, saturates)
  Vpulse=2.0V → P1=1.51x, P10=1.62x (no fire, saturates)
  Vpulse=2.5V → P1=1.66x, P10=1.78x (no fire, saturates)

Problem: pw/tau_E = 1us/1us = 1, each pulse switches ~63% of remaining
polarization → exponential saturation by P3-P5. No gradual integration.

v5 Fix: pw=100ns (pw/tau_E=0.1 → ~10% switching per pulse)
  + Vpulse=3.0, 4.0, 5.0V (enough to exceed Ec through cap divider)
  + N_PULSES=20 (more pulses for gradual integration)
  + Vreset=-4V (reduced from -6V to avoid reset overshoot)
"""
import os

# ── Parameters ──
VPULSE_VAR = "@Vpulse@"   # SWB sweep: 3.0, 4.0, 5.0 V
VRESET = -4.0              # Reduced from -6V to avoid overshoot
VGS_READ = 0.20
VDS = 0.05
PW = 1.0e-7               # 100ns write pulse (10x shorter than v3/v4)
TR = 1.0e-9               # 1ns rise
TF = 1.0e-9               # 1ns fall
T_READ = 1.0e-7           # 100ns read hold
N_PULSES = 20              # More pulses for gradual integration
LEAK_AFTER = 10            # Insert leak gap after pulse 10
...
# ── Header ──
w("*===================================================================")
w("*== PHASE 1B — SIM C v5: SHORT PULSE + HIGH VPULSE FOR GRADUAL LIF")
w("*== v3/v4 showed pw/tau_E=1 saturates in 3 pulses.")
w("*== v5: pw=100ns (pw/tau_E=0.1) → ~10% switching per pulse.")
w(f"*== Sweep: {VPULSE_VAR} = 3.0, 4.0, 5.0 V  (SWB nodes)")
...
# Write output
outpath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sdevice_simC_v5.cmd")
