"""Analyze SimC v5 results: Vpulse sweep 3/4/5V, pw=100ns, Vreset=-4V, N=20.
Generates publication-relevant plots from write-then-read LIF data.
Key change from v3/v4: pw/tau_E = 0.1 (was 1.0) for gradual integration."""
import os
import re
import numpy as np
...
# ── Print summary table ──
print("=" * 90)
print("SimC v5 Results Summary  (pw=100ns, tau_E=1us, pw/tau_E=0.1)")
print("=" * 90)
print(f"{'Node':<10} {'Vpulse':>7} {'ID_base':>12} {'ID_P1':>12} {'P1_ratio':>10}"
      f" {'ID_P10':>12} {'P10_rat':>8} {'ID_P20':>12} {'P20_rat':>8}")
print("-" * 90)
for dirname, r in results.items():
    ...
    print(f"{dirname:<10} {r['Vpulse']:>7.1f} {bl:>12.4e} {p1:>12.4e} {p1/bl:>10.4f}"
          f" {p10:>12.4e} {p10/bl:>8.4f} {p20:>12.4e} {p20/bl:>8.4f}")

print()
print(f"{'Node':<10} {'ID_reset':>12} {'Reset%':>8} {'Fire?':>8} {'FirePulse':>10}")
print("-" * 60)
for dirname, r in results.items():
    bl = r["id_baseline"]
    p20 = r["ids_read"][19]
    pr = r["id_postreset"]
    reset_pct = (1.0 - (pr - bl) / (p20 - bl)) * 100 if (p20 - bl) != 0 else 0
    # Check fire
    ...
    print(f"{dirname:<10} {pr:>12.4e} {reset_pct:>7.1f}% {fire_str:>8} {fire_pulse:>10}")
...
for dirname, r in results.items():
    ...
    inc_str2 = " ".join([f"{x:.4f}" for x in increments[10:]])
    print(f"  {r['Vpulse']:.0f}V P1-10:  {inc_str}")
    print(f"  {r['Vpulse']:.0f}V P11-20: {inc_str2}")
print()
...
LABELS = {
    "n3(3V)": r"$V_{pulse}$=3.0V",
    "n4(4V)": r"$V_{pulse}$=4.0V",
    "n5(5V)": r"$V_{pulse}$=5.0V",
}
...
fig1.tight_layout()
fig1.savefig(os.path.join(OUTDIR, "simC_v5_fig1_integration_fire.png"))
print("Saved: simC_v5_fig1_integration_fire.png")
...
fig2.tight_layout()
fig2.savefig(os.path.join(OUTDIR, "simC_v5_fig2_polarization.png"))
print("Saved: simC_v5_fig2_polarization.png")
...
fig3.tight_layout()
fig3.savefig(os.path.join(OUTDIR, "simC_v5_fig3_dvth.png"))
print("Saved: simC_v5_fig3_dvth.png")
...
ax4a.set_title("Current: Baseline vs P20 vs Post-Reset")
...
fig4.tight_layout()
fig4.savefig(os.path.join(OUTDIR, "simC_v5_fig4_reset.png"))
print("Saved: simC_v5_fig4_reset.png")

# =====================================================================
# PLOT 5: Leak Gap Analysis (current & polarization during gap after P10)
# =====================================================================
...
fig5.tight_layout()
fig5.savefig(os.path.join(OUTDIR, "simC_v5_fig5_leak.png"))
print("Saved: simC_v5_fig5_leak.png")
...
fig6.suptitle(f"SimC v5: Write-Then-Read LIF — Vpulse Sweep "
              f"(pw={PW_NS}ns, τ_E={TAU_E_US}µs, pw/τ_E=0.1)",
              fontsize=14, fontweight="bold", y=1.01)
fig6.tight_layout()
fig6.savefig(os.path.join(OUTDIR, "simC_v5_fig6_summary.png"))
print("Saved: simC_v5_fig6_summary.png")

plt.close("all")
print("\nAll v5 plots generated successfully.")
