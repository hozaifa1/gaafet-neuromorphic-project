*===================================================================
*== PHASE 1A — SIM A: SINGLE PULSE Vth SHIFT
*== Purpose: Apply ONE gate pulse, then read ID-VGS to measure dVth.
*== SWB Sweep: @Vpulse@ = 1.0, 1.5, 2.0, 2.5, 3.0 (5 runs)
*== Fixed: VDS=0.05V, pulse_width=100ns, tau_E=1ns (in .par)
*== Output: ID-VGS curve per Vpulse -> extract dVth per amplitude
*===================================================================
  ...
  * @Vpulse@ is swept via SWB: 1.0, 1.5, 2.0, 2.5, 3.0
  ...
  Transient (
    ...
    Increment=1.4
      ...
      Coupled (Iterations = 100) {Poisson Electron Hole}
      CurrentPlot( Time = (Range=(1e-9 101e-9) Intervals=100) )
  }
  ...
  Transient (
    ...
    Goal { Name="gate_contact" Voltage= 0.0 }
      ...
      Coupled (Iterations = 100) {Poisson Electron Hole}
      CurrentPlot( Time = (Range=(101e-9 102e-9) Intervals=50) )
  }
